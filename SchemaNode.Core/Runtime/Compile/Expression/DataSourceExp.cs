...
/// The data source
/// </summary>
public record DataSource(string App, string Field, NodeType NodeType);

/// <summary>
/// The data source expression
/// </summary>
public record DataSourceExp(DataSource Source) : SchemaExp(Source.NodeType);
                ...
                for (int i = 0; i < inlineFunc.Args.Length; i++)
                {
                    ...
                    if (item == null)
                    {
                        paramMap[arg] = arg.Nullable
                            ? new ConstantExp(arg.NodeType.CreateNode(null)!)
                            : throw new FunctionVisitException(SchemaNodeStatus.FunctionExpWrongFuncArgs);
                    }
                    else
                    {
        ...
        return exp switch
        {
            FieldAccessExp fieldExp => ReplaceExp(fieldExp.Owner) is CollectionItemExp 
                ...
                : Expression.New(typeof(AppSchemaDataFilterValue).GetConstructors()[0], 
                    await context.CompileSchemaExpAsync(new FieldAccessExp(ReplaceExp(fieldExp.Owner), fieldExp.FieldName, fieldExp.NodeType
                    ))),
