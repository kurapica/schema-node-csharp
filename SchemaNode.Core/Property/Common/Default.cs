...
[Relation<Visible, Call>(nameof(Default), $"{NS_SYSTEM_SCHEMA_REFLECT}.{nameof(SystemReflect.isschemakind)}", $"@{nameof(StructFieldSchema.Type)}", true, SCHEMA_KIND_ENUM, SCHEMA_KIND_BOOL, SCHEMA_KIND_INT, SCHEMA_KIND_DECIMAL, SCHEMA_KIND_STRING, SCHEMA_KIND_DATE)]
[Relation<OverrideType, Call>(nameof(Default), $"{NS_SYSTEM_SCHEMA_REFLECT_ARRAY}.{nameof(SystemReflect.Array.getarrayelement)}", $"@{nameof(StructFieldSchema.Type)}")]
