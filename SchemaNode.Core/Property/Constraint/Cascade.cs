...
[Relation<EntrySource, Relation.Assign>(nameof(Cascade), $"{NS_SYSTEM_SCHEMA_REFLECT_ENUM}.{nameof(SchemaNode.Function.Reflect.Enum.getcascades)}", $"@{nameof(StructFieldSchema.Type)}")]
[Relation<Visible, Relation.Call>(nameof(Cascade), NS_SYSTEM_SCHEMA_REFLECT_IS_VALUE_KIND, $"@{nameof(StructFieldSchema.Type)}", SCHEMA_KIND_ENUM)]
