...
/// </summary>
public class AppDataPostgreSqlProvider(NpgsqlConnection dbConn, IServiceProvider serviceProvider, ISqlProvider sqlProvider) : IAppDataSqlProvider<PostgreSqlProvider>
{
    ...
    public async Task<bool> EnsureDynamicTableAsync(DynamicTableSchema schema)
    {
        ...
        string tableNameRaw = schema.AppFieldType.DynamicTableName.Replace("'", "''");
        ...
        try
        {
            ...
            command.CommandText = $"SELECT column_name, data_type, character_maximum_length FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = '{tableNameRaw}' ORDER BY ordinal_position";
            ...
            if (nameTypes.Count > 0)
            {
                ...
                if (!schema.Single)
                {
                    ...
                    if (chkUniqueIndex.Count != uniqueIndex.Count || chkUniqueIndex.Where((p, i) => !p.Equals(uniqueIndex[i])).Any())
                    {
                        ...
                        sb.Add($"CREATE UNIQUE INDEX {refIndex} ON {tableName}({string.Join(',', chkUniqueIndex.Select(sqlProvider.QuoteField))});");
                    }
                }
                ...
                if (schema.Indexes is { Length: > 0 })
                {
                    foreach (var index in schema.Indexes)
                    {
                        ...
                        if (!names.Remove(key))
                        {
                            sb.Add($"CREATE INDEX {sqlProvider.QuoteIndex(key)} ON {tableName}({string.Join(',', schema.ScopeFields.Select(f => sqlProvider.QuoteField(f.Name)).Concat(index.Fields.Select(sqlProvider.QuoteField)))});");
                        }
                    }
                }

                // Remove unused indexes
                foreach (string name in names.Keys.Where(p => !p.Equals(uniqueIndexName, StringComparison.OrdinalIgnoreCase)))
                {
        ...
        if (schema.AppFieldType.Topology == FieldStorageTopology.AttributeBased)
        {
            try
            {
                ...
                string attrTableNameRaw = schema.AppFieldType.AttributeTableName.Replace("'", "''");
                StringBuilder sb = new();
                ...
                command = GetDbCommand();
                command.CommandText = $"CREATE INDEX IF NOT EXISTS {sqlProvider.QuoteIndex($"{attrTableNameRaw}_IDX_TAR_FLD")} ON {tableName}({scopeTargetPart});";
                Logger.LogInformation(command.CommandText);
                await command.ExecuteNonQueryAsync();

                command = GetDbCommand();
                command.CommandText = $"CREATE INDEX IF NOT EXISTS {sqlProvider.QuoteIndex($"{attrTableNameRaw}_IDX_TAR_FLD_INT")} ON {tableName}({scopeTargetPart}, {_refAttrIntField});";
                Logger.LogInformation(command.CommandText);
                await command.ExecuteNonQueryAsync();

                command = GetDbCommand();
                command.CommandText = $"CREATE INDEX IF NOT EXISTS {sqlProvider.QuoteIndex($"{attrTableNameRaw}_IDX_TAR_FLD_STR")} ON {tableName}({scopeTargetPart}, {_refAttrStrField});";
                Logger.LogInformation(command.CommandText);
                await command.ExecuteNonQueryAsync();

                command = GetDbCommand();
                command.CommandText = $"CREATE INDEX IF NOT EXISTS {sqlProvider.QuoteIndex($"{attrTableNameRaw}_IDX_TAR_FLD_DAT")} ON {tableName}({scopeTargetPart}, {_refAttrDatField});";
                Logger.LogInformation(command.CommandText);
                await command.ExecuteNonQueryAsync();
            }
            catch (Exception ex)
            ...
            try
            {
                if (reader.HasRows)
                {
                    while (await reader.ReadAsync())
                    {
                        ...
                        // jsonb — Npgsql returns JSONB as string
