...
public class AppDataMySqlProvider(MySqlConnection dbConn, IServiceProvider serviceProvider, ISqlProvider sqlProvider, ISchemaContext context) : IAppDataSqlProvider<MySqlProvider>, IAsyncDisposable
{
        ...
        else
        {
            ...
            try
            {
                if (reader.HasRows)
                {
                    while (await reader.ReadAsync())
                    {
                        var pack = type == AppSchemaDataResult.Field
            ...
            try
            {
                if (reader.HasRows)
                {
                    while (await reader.ReadAsync())
                    {
                        ...
                        foreach (DynamicTableField tableField in schema.Fields.Where(p => p.Primary))
                        {
                            var val = tableField.FromReader(reader, offset++);
            ...
            bool canAdd = true, bool onlyAdd = false, string[]? overrides = null)
    {
        ...
        if (schema.Single)
        {
            ...
            (var origin, _) = await QueryDynamicTableAsync(schema, AppSchemaDataResult.First);
            ...
            else if (value is StructNode pack)
            {
                ...
                if (!isInsert)
                {
                    ...
                    // Body
                    bool preCond = false;
                    foreach ((string fld, var val) in schema.GetFieldValues(pack))
                    {
        ...
        else
        {
            ...
            // Query
            var origin = await this.QueryOriginNodesAsync(schema, packs, forUpdate: true);
            ...
            if (oArr is { Count: > 0 })
            {
                foreach (StructNode obj in oArr.Cast<StructNode>())
                {
                    keys.Clear();
                    bool fullFill = true;
                    foreach ((_, var v) in schema.GetFieldValues(obj, true))
                    {
                        // Check value
                        if (v == null || v.IsEmpty)
                        {
            ...
            foreach (StructNode pack in packs)
            {
                ...
                sb.Clear();
                sb.Append(wherePrefix);
                foreach ((string fld, var v) in schema.GetFieldValues(pack, true))
                {
                    // Check value
                    if (v == null || v.IsEmpty)
                    {
                ...
                if (!isInsert && (!onlyAdd || overrides is { Length: > 0 }))
                {
                    ...
                    // Body
                    bool preCond = false;
                    foreach ((string fld, var v) in schema.GetFieldValues(pack, false, true))
                    {
                        // Check override
    ...
    public async Task<(bool result, IValueAccess? origin)> DeleteDynamicTableDataAsync(DynamicTableSchema schema, AppSchemaDataFilter? filter)
    {
        ...
        if (schema.Single)
        {
            (var origin, _) = await QueryDynamicTableAsync(schema,AppSchemaDataResult.First, forUpdate: true);
        ...
        else
        {
            ...
            var (origin, _) = await QueryDynamicTableAsync(schema, AppSchemaDataResult.List, filter, forUpdate: true);
    ...
    public async Task<(bool result, IValueAccess? origin)> ClearDynamicTableDataAsync(DynamicTableSchema schema)
    {
        ...
        var (origin, _) = await QueryDynamicTableAsync(schema, schema.Single ? AppSchemaDataResult.First : AppSchemaDataResult.List, forUpdate: true);
    ...
    async Task<StructFieldSchema[]> GetStructFieldConfigs(AppFieldType appField, StructNode node, RelationType relation)
    {
        ...
        for (int i = 0; i < call.Args.Length; i++)
        {
            ...
            if (!string.IsNullOrEmpty(arg.Source))
            {
                ...
                else if (_relationDataCache.TryGetValue(fieldName, out var cache) && cache != null)
                {
                    args[i] = cache;
                }
                else
                {
                    ...
                    (var result, int total) = await QueryDynamicTableAsync(schema,AppSchemaDataResult.List);
                    if (total > 50)
    ...
    async Task SaveAttributeBasedFieldAsync(string attrTable, Dictionary<string, string> scopeItems, StructFieldSchema[] fields, JsonObject? value, string prev, List<(string k, IValueAccess v)> primaries)
    {
        ...
        foreach (StructFieldSchema field in fields.Where(f => f.GetProperty<DisplayOnly>()?.Value != true))
        {
            ...
            // For one field value
            var node = r != null ? type.From(r) : null;
    ...
    (string where, Dictionary<string, string> scopeItems) PrepareWhere(DynamicTableSchema schema, string prefix = "")
    {
        ...
        foreach ((string item, var value)  in schema.GetScopeItems(_context))
        {
            if (value == null || value.IsEmpty)
    ...
    private readonly ConcurrentDictionary<string, IValueAccess?> _relationDataCache = new (StringComparer.OrdinalIgnoreCase);
